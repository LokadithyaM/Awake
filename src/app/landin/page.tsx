...
export default function LandingPage() {
    ...
    useEffect(() =>{
        async function fetchKnown(){
            try{
                ...
                console.error("Error fetching known users:");
            }
        }
    ...
    return (
        <div className="min-h-screen w-full bg-[#121212] border-2 border-black flex items-center justify-center gap-2">
            <div className="w-[1300px] h-[1300px] border-2 border-black flex flex-col items-center gap-2 p-2">
                ...
                <div className="min-h-screen w-full border-2 bg-transparent border-black flex items-start gap-2 p-2">
                    ...
                    <div className="w-[800px] min-h-screen max-h-screen border-2 border-black flex flex-col items-center gap-1 overflow-auto shadow-white shadow-xl">
                        <div className="w-full h-[200px] border-2 border-black shadow-white shadow-xl flex flex-col gap-5 p-2">
                            ...
                            <div className="flex justify-end">
                                <button onClick={handleClick} className="w-[50px] h-[50px] border-2 border-black mt-[60px]">
                                    <img src="/conatact2.svg" alt="SVG Icon" className="w-full h-full" />
                                </button>
                            </div>
                        </div>
                    ...
                    <div className="w-full h-full bg-transparent text-white border-2 border-black">
                        {selectedProfile ? (
                            <div className="w-[800px] h-full bg-transparent text-whiteborder-2 border-black">
                                <div className="w-full border-2 border-black bg-transparent flex text-1xl font-extrabold tracking-tight text-gray-500 items-center">
                                    <div>
                                ...
                                <div className="w-full h-[730px] border-2 border-black text-black mb-2">
                                    <Chat 
                                ...
                                <div className="w-[750px] h-[60px] bg-[#262626] rounded-xl flex items-center px-4">
                                    <input
                                        type="text"
                                        value={message ?? ""}
                                        ...
                                        className="border-none outline-none bg-[#262626] text-lg flex-1 text-green-800"
                                        placeholder="Type a message..."
                                        required
                                    />

                                    <button onClick={handlePost} className="w-[50] h-[50] rounded-full">
                                        <img src="/sent.svg" alt="SVG Icon" className="w-full h-full" />
                                    </button>
                                </div>
                                </div>
                            </div>
    ...
    {messages.map((msg, index) => {
      ...
      return (
        <div key={index} className={`flex ${isSender ? "justify-end" : "justify-start"}`}>
          <div
            className={`p-2 border rounded-lg max-w-[75%] text-gray-200 ${isSender ? " bg-[#1E1E1E]" : "text-black bg-[#262626]"}`}
          >
...
function ProfileCard({ profile, toggleDetails, handleAddProfile, handleProfileClick }: { 
    ...
    return (
        <div className="group w-full text-black bg-[#121212] h-auto border-2 border-gray-800 flex flex-col items-center p-2 cursor-pointer shadow-white shadow-lg rounded-xl">
            {!profile.showDetails ? (
                <div onClick={() => handleProfileClick(profile)} className="w-full h-[90px] border-2 border-black flex items-center p-2 justify-center">
                    {/* <div className="w-[60px] h-[60px] border-2 border-black rounded-full mr-[20px] bg-white"></div> */}
                    ...
                    <button onClick={() => toggleDetails(profile.id)} className="w-[40px] h-[40px] border-2 border-black rounded-xl flex items-center justify-center">
                        <img src="/cog.svg" alt="SVG Icon" className="w-8 h-8 text-green-500" />
                    </button>
                </div>
            ) : (
